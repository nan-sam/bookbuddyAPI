...
const getUsersReservations = async (userId) => {
  try {
    const SQL = `SELECT reservations.id, books.title, books.description, books.coverimage, books.author 
    FROM reservations JOIN books ON reservations.bookid = books.id AND userid=$1`;
